...
def stage2_calculations(code_entered, three_dimensional_arr):
    ...
    flattened_array = (
        subarray_view.flatten()
    )  # Flatten the subarray to consider all enrollments
    filtered_array = flattened_array[
        flattened_array > 500
    ]  # Filter the flattened array to include only values over 500
    if filtered_array.size == 0:
        ...
        print(
            "For all enrollments over 500, the median was:",
            median_enrollment.astype(int),
        )
...
def stage3_calculations():
    ...
    mean_2013 = np.nanmean(all_years_list[0])
    ...
    # Gets every 3rd value for the last year starting with the 3rd value and sums them
    total_2022 = np.nansum(all_years_list[-1][2::3])
...
def main():
    ...
    while True:
        ...
        try:
            ...
            stage2_calculations(
                school_code1, three_dimensional_arr
            )  # Calls stage2_calculations
